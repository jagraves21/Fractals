...
public class SquareFlake extends LSystem
{
	public SquareFlake()
	{
		super();
	...
	{
		return new StringBuilder("F[+F[+F]--F]-F[-F]++F-F");
	}
	...
	public StringBuilder applyTransition(StringBuilder curve)
	{
		...
		for(int ii=0; ii < curve.length(); ii++)
		{
			...
			{
				nextCurve.append("F[+F[+F]--F]-F[-F]++F-F");
			}
			else
			{
	...
	public static void main(String[] args)
	{
		...
		FractalPanel fractal = new FractalPanel(new SquareFlake());		
